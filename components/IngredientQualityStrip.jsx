...
export default function IngredientQualityStrip() {
  ...
  const t = translations?.ingredientQualityStrip;

  const items = [
    {
      title: t?.items?.[0]?.title || "Quality-First Manufacturing",
      desc:
        t?.items?.[0]?.desc ||
        "GMP-aligned processes with strong quality systems and traceability.",
    },
    {
      title: t?.items?.[1]?.title || "Documentation Support",
      desc:
        t?.items?.[1]?.desc ||
        "COA, MSDS and supporting docs available as per product requirements.",
    },
    {
      title: t?.items?.[2]?.title || "Reliable Global Supply",
      desc:
        t?.items?.[2]?.desc ||
        "Flexible quantities and export support for long-term partnerships.",
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-16">
        <div className="grid gap-2 md:grid-cols-3">
          {items.map((item, index) => (
            <div
              key={index}
              className="relative bg-white border border-[#0d2d47] shadow-sm hover:shadow-md transition h-[220px] max-w-[280px] mx-auto flex flex-col items-center justify-center px-6 py-8 text-center"
            >
              <div className="h-[2px] w-12 bg-[#FF7A00] mb-6 self-start ml-2" />

              <h3 className="text-[#0d2d47] font-bold text-lg">
                {item.title}
              </h3>

              <p className="mt-3 text-sm text-[#0d2d47]/80">
                {item.desc}
              </p>
